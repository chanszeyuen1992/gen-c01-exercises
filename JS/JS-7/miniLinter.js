...
console.log(`The word count: ${storyArray.length}`);

let betterWords = storyArray.filter(function(word) {
...
for (let i = 0; i < overusedWords.length; i++) {
    for (let j = 0; j < storyArray.length; j++) {
        if (storyArray[j] == overusedWords[i]) {
            acc++;
        }
    }
}
...
console.log(`The number of times each overused word appears: ${acc}`)
